...
export class AuthenticationService extends ExternalResourceService {
    ...
    login(username: string, password: string): Observable<boolean> {
        ...
        if (Config.ENV === 'PROD') {
            ...
            return this.http.post(Config.API + '/oauth/token', body.toString(), options)
                .map((response: any) => {
            ...
            return this.http.get<any>('/assets/mock-data/token-data.json')
                .map((response: any) => {
                    // login successful if there's a jwt token in the response
                    if (response.access_token && response.refresh_token) {
                        // set token property
    ...
    logout(): Observable<boolean> {
        ...
        if (Config.ENV === 'PROD') {
            ...
            return this.http.post(Config.API + '/oauth/revoke-token', null, options)
                .map((response: any) => {
                    // login successful if there's a jwt token in the response
