...
/**
 * This class represents the lazy loaded ConfigurationChoiceComponent.
 */
@Component({
  moduleId: module.id,
  selector: 'fix-configuration-choice',
  templateUrl: 'configurations-choice.component.html',
  styleUrls: ['configurations-choice.component.css']
})
export class ConfigurationsChoiceComponent implements AfterViewInit, OnInit {
  ...
  setCollapseBodiesHeight() {
    ...
    this.collapsedEelementsStatus.forEach((value: boolean, key: string) => {
      ...
      if (activeCollapse === key) {
        const wrapperHeight = jQuery('.details-choice-config').innerHeight();
